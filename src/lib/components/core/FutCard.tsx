...
import {getHexColor, getPlayerDetailRoute, getPlayStyleSVGPath} from "@/lib/utils";
import {TiStarFullOutline} from "react-icons/ti";
import Link from "next/link";
import { examplePlayerData } from '@/lib/utils/data';


export default function FutCard(props: FutCardPropTypes) {
    ...
    return <Link
        href = {getPlayerDetailRoute({futId: futid, playerId: playerid, year})}
        className={`${layout} w-full bg-cover relative aspect-[0.71527778] hidden cursor-pointer`}
        style={{width, fontSize}} ref={containerRef}>
        <Image width={0} height={0}
               className={`fut-card-bg absolute w-full h-full top-0 left-0 `}
               src={background_img_url_l} alt={`card-bg`}/>
        <Image
            width={0} height={0}
            className={`absolute ${isdynamicportrait ? 'player-avatar-dynamic' : 'player-avatar'}`}
            src={player_img_url}
            alt="player_avatar"/>
        <div
